...
# DBI::dbWriteTable(NBAdb, "GameLogsAdj", df)
...
# DBI::dbWriteTable(NBAdb, "ResultsBook", df)
# DBI::dbWriteTable(NBAdb, "Plays", df)
# DBI::dbWriteTable(NBAdb, "Odds", df)
...
df <- df %>%
    collect() %>% mutate(date = as.Date(df$date, origin ="1970-01-01"))

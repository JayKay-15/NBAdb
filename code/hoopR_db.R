...
#          mar=c(0,0,1,0))
cor_win <- cor(nba_cor, nba_cor$team_winner)
cor_win <- as.matrix(cor_win[order(abs(cor_win[,1]), decreasing = T),])
cor_win


combos <- findLinearCombos(nba_final %>%
                               select(away_implied_prob, home_implied_prob,
                                      away_fg2m:away_pace,
                                      home_fg2m:home_pace,
                                      b2b_first:opp_b2b_second))

train <- nba_final %>%
    filter(season %in% c(2014:2021)) %>%
    select(team_winner,
           away_implied_prob, home_implied_prob,
           away_fg2m:away_pace,
           home_fg2m:home_pace,
           b2b_first:opp_b2b_second) %>%
    select(-c(combos$remove+1))
...
confusionMatrix(test$team_winner, factor(ifelse(win_pred[,1] > 0.5, "win", "loss"),
                                         levels = c("win","loss")), positive = "win")

away <- as.numeric(win_pred[,1])
home <- as.numeric(win_pred[,2])
obs <- test$team_winner
pred <- factor(ifelse(away > 0.5, "win", "loss"), levels = c("win","loss"))
obs_pred <- data.frame(obs = obs,
                       pred = pred,
                       away = away, 
                       home = home,
                       act = ifelse(obs == "win", 1, 0)) # this needed?
